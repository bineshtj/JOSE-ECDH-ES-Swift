...
public struct EcdhEsJweHeader: JSONWebEncryptionHeader {
    ...
    public init(cloneFrom: JSONWebEncryptionHeader) throws {
         try self.init(parameters: cloneFrom.allParameters())
    }
    ...
    public subscript(index: String) -> Any? {
        ...
        set {
            ...
            case "epk":
                ...
                guard newValue == nil || newValue is ECPublicKey,
                    let epk = newValue as? ECPublicKey,
                    let epkJsonData = epk.jsonData()
                else {
                    return
                }
                do {
                    let epkDic = try JSONSerialization.jsonObject(with: epkJsonData, options: [.mutableContainers])
